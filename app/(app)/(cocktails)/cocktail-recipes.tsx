...
export default function CocktailRecipesScreen() {
  const { t } = useI18n();
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    return subscribeToCocktailRecipes(user.id, setRecipes);
  }, [user]);
  ...
  const renderRecipe = ({ item }: { item: CocktailRecipe }) => {
    ...
    return (
      <Pressable
        onPress={() => openRecipe("/(app)/cocktail-recipe-details", item)}
        style={({ pressed }) => [styles.card, pressed && styles.buttonPressed]}
      >
        <View style={styles.cardIcon}>
          <MaterialCommunityIcons name="glass-cocktail" color={colors.primary} size={24} />
        </View>
        <View style={styles.cardContent}>
          <View style={styles.cardTitleRow}>
            ...
            <Pressable
              onPress={(event) => {
                ...
                openRecipe("/(app)/edit-cocktail-recipe", item);
              }}
          ...
          <View style={styles.metaRow}>
            <View style={styles.metaPill}>
              <MaterialCommunityIcons name="flash-outline" color={colors.accent} size={13} />
              <Text style={styles.metaText}>
                {t("recipes.calories_value", { calories: item.calories })}
              </Text>
            </View>
            <View style={styles.metaPill}>
              <MaterialCommunityIcons name="glass-cocktail" color={colors.textMuted} size={13} />
              <Text style={styles.metaText}>
          ...
          <View style={styles.ingredientSummary}>
            <Text style={styles.groupLabel}>{t("cocktail_recipes.available")}</Text>
            ...
            <Text style={styles.groupLabel}>{t("cocktail_recipes.missing")}</Text>
            <View style={styles.chips}>
  ...
  return (
    <View style={styles.container}>
      <FlatList
          ...
          <View style={styles.header}>
            <View style={styles.titleRow}>
              <View style={styles.titleBlock}>
                <Text style={styles.title}>{t("cocktail_recipes.title")}</Text>
            ...
            <View style={styles.searchBar}>
              ...
              <TextInput
                ...
                placeholder={t("cocktail_recipes.search_placeholder")}
        ...
        ListEmptyComponent={
          <View style={styles.emptyState}>
            <MaterialCommunityIcons name="glass-cocktail" color={colors.muted} size={46} />
            <Text style={styles.emptyTitle}>
              {searchQuery ? t("recipes.empty_filtered") : t("cocktail_recipes.empty")}
            </Text>
            <Text style={styles.emptySubtitle}>
              {searchQuery ? t("wardrobe_list.empty_filtered_subtitle") : t("cocktail_recipes.empty_subtitle")}
            </Text>
          </View>
        }
        ListFooterComponent={
      ...
      <Pressable
        onPress={() => router.push("/(app)/add-cocktail-recipe")}
        style={({ pressed }) => [
          styles.fab,
        ...
        accessibilityRole="button"
        accessibilityLabel={t("cocktail_recipes.add_button")}
      >
...
const createStyles = (theme: AppTheme) => {
  const colors = theme.colors;
  const accentDim = theme.isDark ? "rgba(59, 164, 245, 0.12)" : "rgba(59, 164, 245, 0.1)";
  ...
  return StyleSheet.create({
    ...
    searchBar: { minHeight: 44, flexDirection: "row", alignItems: "center", gap: spacing.xs, paddingHorizontal: spacing.md, borderRadius: 999, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface2 },
    searchInput: { flex: 1, color: colors.text, fontSize: 14, lineHeight: 20, paddingVertical: spacing.xs },
    separator: { height: spacing.sm },
    card: { flexDirection: "row", gap: spacing.sm, padding: spacing.sm, borderRadius: 18, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface2 },
    cardIcon: { width: 56, height: 56, alignItems: "center", justifyContent: "center", borderRadius: 14, backgroundColor: colors.surface3 },
    cardContent: { flex: 1, gap: spacing.xs },
    cardTitleRow: { flexDirection: "row", alignItems: "center", gap: spacing.xs },
    recipeName: { flex: 1, color: colors.text, fontSize: 15, lineHeight: 20, fontWeight: "700" },
    editButton: { width: 32, height: 32, alignItems: "center", justifyContent: "center", borderRadius: 16, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface3 },
    metaRow: { flexDirection: "row", flexWrap: "wrap", gap: spacing.xs },
    metaPill: { minHeight: 24, flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: spacing.xs, borderRadius: 12, backgroundColor: colors.surface3 },
    metaText: { color: colors.textMuted, ...typography.caption, fontWeight: "600" },
    ingredientSummary: { gap: spacing.xs },
    groupLabel: { color: colors.textMuted, ...typography.caption, textTransform: "uppercase", letterSpacing: 0.6, fontWeight: "700" },
    chips: { flexDirection: "row", flexWrap: "wrap", gap: spacing.xs },
    availableChip: { maxWidth: "100%", paddingVertical: 5, paddingHorizontal: spacing.sm, borderRadius: 999, color: colors.accent, backgroundColor: theme.isDark ? "rgba(0, 230, 118, 0.12)" : "#F6FFED", ...typography.caption, fontWeight: "700" },
    missingChip: { maxWidth: "100%", paddingVertical: 5, paddingHorizontal: spacing.sm, borderRadius: 999, color: colors.danger, backgroundColor: theme.isDark ? "rgba(255, 71, 87, 0.12)" : "#FFF2F0", ...typography.caption, fontWeight: "700" },
    mutedChip: { color: colors.textMuted, ...typography.caption },
    shoppingButton: { minHeight: 36, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: spacing.xs, marginTop: spacing.xs, borderRadius: 10, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface3 },
    shoppingButtonText: { color: colors.primary, ...typography.caption, fontWeight: "700" },
    ...
    pagination: { flexDirection: "row", alignItems: "center", gap: spacing.xs, marginTop: spacing.lg },
    pageButton: { minHeight: 38, flexDirection: "row", alignItems: "center", gap: 2, paddingHorizontal: spacing.sm, borderRadius: 19, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.surface2 },
    pageText: { color: colors.primary, ...typography.caption, fontWeight: "700" },
    pageStatus: { flex: 1, color: colors.textMuted, textAlign: "center", ...typography.caption },
    fab: { position: "absolute", right: 20, width: 52, height: 52, alignItems: "center", justifyContent: "center", borderRadius: 26, backgroundColor: "#3BA4F5", shadowColor: "#000", shadowOpacity: theme.isDark ? 0.6 : 0.18, shadowRadius: 24, shadowOffset: { width: 0, height: 12 }, elevation: 6 },
